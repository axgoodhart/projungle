...
function renderFileRow(projectId, file, isCover = false) {
  ...
  return `
    <div class="file-row ${isCover ? 'is-cover' : ''}" data-action="open-file" data-project-id="${projectId}" data-file-id="${file.id}" title="${escapeAttr(file.path)}">
      ...
      ${file.previewable ? `<button class="tiny-btn cover-btn ${isCover ? 'active' : ''}" data-action="set-cover" data-project-id="${projectId}" data-file-id="${file.id}" title="${isCover ? 'Current cover' : 'Set as cover'}">⊞</button>` : ''}
    ...
    >
      ...
      ${hasCover ? `
        <div class="project-cover" data-role="cover-drag-area">
          <img class="project-cover-img" src="${escapeAttr(toFileUrl(coverFile.path))}" alt="" loading="lazy"
               style="object-position:${project.coverPosition}" />
...
function renderViewer(project) {
  ...
  /* hero images */
  const heroImages = project.files.filter((f) => f.previewable).slice(0, 5);
  ...
  return `
    ...
    <div class="viewer-content" style="--accent:${project.accent}; --accent-rgb:${project.accentRgb};">
      ...
      <div class="viewer-hero ${heroImages.length ? '' : 'viewer-hero--empty'}">
        ${heroImages.length
          ? heroImages.map((f) => `<img class="viewer-hero-img" src="${escapeAttr(toFileUrl(f.path))}" alt="" loading="lazy" />`).join('')
          : '<div class="viewer-hero-pattern"></div>'}
        <div class="viewer-hero-fade"></div>
      </div>
...
world.addEventListener('pointerdown', (event) => {
  ...
  if (coverArea) {
    ...
    if (!project) return;
    const coverImg = coverArea.querySelector('.project-cover-img');
    if (!coverImg) return;
...
dimToggle.addEventListener('click', () => {
  document.querySelector('.app-shell').classList.toggle('dim-mode');
});
