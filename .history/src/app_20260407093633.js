...
async function handleDrop(event) {
  ...
  try {
    ...
    const dropTarget = document.elementFromPoint(e.clientX, e.clientY)?.closest('.project-card');
...
window.addEventListener('pointermove', (e) => {
  ...
  if (interaction.kind === 'pan') {
    state.viewport.x = interaction.startX + (e.clientX - interaction.startClientX);
    state.viewport.y = interaction.startY + (e.clientY - interaction.startClientY);
                updateCursorPosition(e.clientX, e.clientY);
...
document.addEventListener('mousemove', (event) => {
  ...
  const pct = clamp(((e.clientX - drag.trackLeft) / drag.trackWidth) * 100, 0, 100);
  node.style.left = `${pct}%`;
...
document.addEventListener('mouseup', (event) => {
  ...
  const finalPct = clamp(((e.clientX - drag.trackLeft) / drag.trackWidth) * 100, 0, 100);
